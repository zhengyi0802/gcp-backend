...
@section('content')
    <form id="startpage-form" action="{{ route('resellers.update', $startpage->id) }}" method="POST">
    @csrf
    @method('PUT')
    <input name="p" value="startpage" type="hidden" >
    ...
    <div class="row col-12">
        <div class="card-group col-md-12">
            ...
            <x-adminlte-select name="mime_type" label="{{ __('startpages.mime_type') }}" fgroup-class="col-md-6"
             ...
             <option value="i_video" {{ ($startpage->mime_type == "i_video") ? "selected" : null }} >{{ __('startpages.type_video') }}</option>
    ...
    <div class="row col-12">
      <div class="card-group col-md-12" >
        ...
        <input type="radio" name="status" value="1" {{ ($startpage->status==1) ? "checked":null }} >{{ __('status.enabled') }}
        <input type="radio" name="status" value="0" {{ ($startpage->status!=1) ? "checked":null }} >{{ __('status.disabled') }}
      </div>
    </div>
    <div class="row col-12">
