...
@section('content')
    <form id="user-project-form" action="{{ route('users.project.updatepermission', ['pid' => $permission->id]) }}" method="POST">
    @csrf
    <div class="row col-12">
      <div class="card-group col-md-12">
        <x-adminlte-input name="email" label="{{ __('users.email') }}" fgroup-class="col-md-6"
          value="{{ $permission->user->email  }}" disabled />
        <x-adminlte-input name="project" label="{{ __('users.project') }}" fgroup-class="col-md-6"
          value="{{ $permission->project->name }}" disabled />
      </div>
    </div>
    <div class="row col-12">
      <div class="card-group col-md-12">
